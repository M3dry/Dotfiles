...
struct Client {
	...
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, needresize, iscentered, issticky, isterminal, noswallow;
    int issteam;
	pid_t pid;
	Client *next;
	Client *snext;
...
focus(Client *c)
{
	...
	if (c) {
		...
		if(c->isfloating)
			...
			XSetWindowBorder(dpy, c->win, scheme[SchemeSel][ColBor].pixel);
		setfocus(c);
	} else {
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if(c->isfloating)
		...
		XSetWindowBorder(dpy, w, scheme[SchemeNorm][ColBor].pixel);
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating)
		...
		XSetWindowBorder(dpy, selmon->sel->win, scheme[SchemeSel][ColBor].pixel);
	if(selmon->sel->isfloating)
...
unfocus(Client *c, int setfocus)
{
	if (!c)
		return;
	grabbuttons(c, 0);
	XSetWindowBorder(dpy, c->win, scheme[SchemeNorm][ColBor].pixel);
	if (setfocus) {
		XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
		XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
	}
